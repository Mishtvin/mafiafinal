...
export function VideoConferenceClient(props: {
  ...
  // Обработчик событий для комнаты LiveKit и взаимодействия с браузером
  useEffect(() => {
    if (!room) return;
    
    // Обработчик отключения от комнаты
    const handleRoomDisconnect = () => {
      console.log('Обработчик roomDisconnected вызван - отключаемся от WebSocket');
      
      // Если есть активный WebSocket (в состоянии slotsState), закрываем его
      if (slotsState && slotsState.disconnect) {
        slotsState.disconnect();
      }
    };
    
    // Обработчик закрытия страницы/вкладки
    const handleBeforeUnload = () => {
      console.log('Вкладка закрывается - выполняем корректное отключение');
      
      try {
        // Отключаемся от комнаты LiveKit
        if (room && room.state === 'connected') {
          console.log('Отключаем комнату LiveKit');
          room.disconnect();
        }
        
        // Закрываем WebSocket, если он есть
        if (slotsState && slotsState.disconnect) {
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'hidden') {
        console.log('Вкладка скрыта - готовим к возможному отключению');
        // Не отключаемся полностью, а отмечаем в sessionStorage
        window.sessionStorage.setItem('tab-hidden-timestamp', Date.now().toString());
      } else if (document.visibilityState === 'visible') {
        console.log('Вкладка снова активна');
        // Удаляем отметку о скрытости
        window.sessionStorage.removeItem('tab-hidden-timestamp');
      }
    };
    ...
    return () => {
      ...
      // При размонтировании компонента также отключаемся
      try {
        if (room && room.state === 'connected') {
          console.log('Отключаемся от комнаты при размонтировании компонента');
          room.disconnect();
        }
        
        // Закрываем WebSocket соединение
        if (slotsState && slotsState.disconnect) {
          console.log('Закрываем WebSocket при размонтировании компонента');
          slotsState.disconnect();
        }
      } catch (e) {
        console.error('Ошибка при отключении в cleanup:', e);
      }
    };
  }, [room, slotsState]);
